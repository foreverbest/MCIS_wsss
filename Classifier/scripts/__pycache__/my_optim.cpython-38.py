...
# Embedded file name: F:\code\MCIS_wsss\classifier\scripts\my_optim.py
# Compiled at: 2021-02-07 20:00:18
...
class PolyOptimizer(optim.SGD):
    ...
    def __init__(self, params, lr, weight_decay, max_step, momentum=0.9):
        ...
        self._PolyOptimizer__initial_lr = [group['lr'] for group in self.param_groups]

    def step(self, closure=None):
        if self.global_step < self.max_step:
            lr_mult = (1 - self.global_step / self.max_step) ** self.momentum
            for i in range(len(self.param_groups)):
                self.param_groups[i]['lr'] = self._PolyOptimizer__initial_lr[i] * lr_mult
...
def adjust_lr(args, optimizer, epoch):
    if 'cifar' in args.dataset:
        ...
        if 'indoor' in args.dataset:
            change_points = [
             60, 80, 100]
        else:
            if 'dog' in args.dataset:
                change_points = [
                 60, 80, 100]
            else:
